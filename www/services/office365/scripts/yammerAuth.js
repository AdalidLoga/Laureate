...
(function (yammerAuth) {
    var Token = (function () {
        function Token(idToken, context, resourceId, clientId, redirectUri) {
           // var encoded_idToken = idToken.split('.')[1].replace('-', '+').replace('_', '/'), decoded_idToken = Microsoft.Utility.decodeBase64AsString(encoded_idToken);
            ...
            this._clientId = clientId || yammerAuth.Settings.clientId;
            this._redirectUri = redirectUri || yammerAuth.Settings.redirectUri;
            this._resourceId = resourceId;

            this._accessToken = context._cacheManager._cache_entry[clientId].access_tokens["https://www.yammer.com/lagash.com/oauth2/authorize"].token;
        }
        Token.prototype.getDeferred = function () {
            if (yammerAuth.deferred) {
                return yammerAuth.deferred();
            }
    ...
    var Context = (function () {
        function Context(authYammerUri, redirectUri) {
            this._accessToken;
            this._redirectUri = 'http://localhost/';
            this._cacheManager = new CacheManager();
            if (!authYammerUri) {
                if (yammerAuth.Settings.authYammerUri) {
                    this._authUri = yammerAuth.Settings.authYammerUri;
                } else {
                    throw new Microsoft.Utility.Exception('No authYammerUri provided nor found in yammerAuth.authYammerUri');
                }
            } else {
                this._authUri = authYammerUri;
            }
            if (this._authUri.charAt(this._authUri.length - 1) !== '/') {
                this._authUri += '/';
            }
            if (!redirectUri) {
                if (yammerAuth.Settings.redirectUri) {
                    this._redirectUri = yammerAuth.Settings.redirectUri;
                }
            } else {
        ...
        Context.prototype.isLoginRequired = function (resourceId, clientId) {
            if (!clientId) {
                if (yammerAuth.Settings.clientId) {
                    clientId = yammerAuth.Settings.clientId;
                } else {
                    throw new Microsoft.Utility.Exception('clientId was not provided nor found in yammerAuth.clientId');
                }
            }
        ...
        Context.prototype.getAccessToken = function (resourceId, loginHint, clientId, redirectUri) {
            ...
            if (!clientId) {
                if (yammerAuth.Settings.clientId) {
                    ...
                    deferred.reject(new Microsoft.Utility.Exception('clientId was not provided nor found in yammerAuth.clientId'));
                    return deferred;
                }
            }
            ...
            var onRedirect = function (e) {
                ...
                if (loadUri.substr(0, redirectUri.length).toLowerCase() === redirectUri.toLowerCase()) {
                    ...
                    var code = this.getParameterByName(loadUri, 'code');
                    var error = this.getParameterByName(loadUri, 'error_description');

                    if (code) {
                        //var url = this._authUri + 'oauth2/access_token.json?' + '&client_id=' + clientId + '&client_secret=K1Mj6WXnA3fk7bF9S4hxfasxHiiguqKkE2LE09tiA&code=8aQbfRST4qG47oLr4ukz2g' +  '&code=' + code, data = '';
                        var url = this._authUri + 'oauth2/access_token.json?' + '&client_id=' + clientId + '&client_secret=VXrsaAkORUjVg5BoroXjHHI3bZeaNUrRiKOb3YMBw8g&code=8aQbfRST4qG47oLr4ukz2g' + '&code=' + code, data = '';
                        this.post(url, data).then(function (result) {
                            var jsonResult = JSON.parse(result), access_token = {
                                token: jsonResult.access_token,
                                expires_in: new Date((new Date()).getTime() + (jsonResult.expires_in - 300) * 1000)
                            ...
                            this._cacheManager.setAccessToken(clientId, resourceId, access_token);
                            this._cacheManager.setIdToken(clientId, jsonResult.access_token.user_id);
                            //this._cacheManager.setRefreshToken(clientId, jsonResult.refresh_token);
                            this._accessToken = access_token.token.token;
                            deferred.resolve(access_token.token);
                        }.bind(this), function (xhr) {
                            deferred.reject(new Microsoft.Utility.HttpException(xhr));
                        });
            ...
            var ref = window.open(authorizationUri, '_blank', 'location=yes,clearsessioncache=yes');
    ...
    yammerAuth.Context = Context;
})(yammerAuth || (yammerAuth = {}));
//# sourceMappingURL=yammerAuth.js.map
